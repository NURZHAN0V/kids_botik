...
def обработать_ход_крестики_нолики(user_id, координаты_str):
    ...
    try:
        ...
        if проверить_победу(игра['поле'], "X"):
            del игры_крестики_нолики[user_id]
            поле_str = вывести_поле_строкой(игра['поле'])
        ...
        if свободные:
            ...
            if проверить_победу(игра['поле'], "O"):
                del игры_крестики_нолики[user_id]
                поле_str = вывести_поле_строкой(игра['поле'])
                return f"{поле_str}\n💀 Победил компьютер\nДля новой игры напиши 'крестики-нолики'"
        
        if not свободные or len(свободные) == 1:
            del игры_крестики_нолики[user_id]
            поле_str = вывести_поле_строкой(игра['поле'])
            return f"{поле_str}\n🤝 Ничья!\nДля новой игры напиши 'крестики-нолики'"
...
def проверить_победу(поле, символ):
    ...
    for i in range(3):
        if all(поле[i][j] == символ for j in range(3)): return True
        if all(поле[j][i] == символ for j in range(3)): return True
    if all(поле[i][i] == символ for i in range(3)): return True
    if all(поле[i][2-i] == символ for i in range(3)): return True
    return False

def вывести_поле_строкой(поле):
