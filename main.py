...
def создать_игру_морской_бой(user_id):
    ...
    поле = [["🌊" for _ in range(10)] for _ in range(10)]
    корабли = []
    
    # Размещение кораблей
    for размер_корабля in флот:
        размещён = False
        попытки = 0
        while not размещён and попытки < 100:
            попытки += 1
            x = random.randint(0, 9)
            y = random.randint(0, 9)
            горизонтально = random.choice([True, False])
            
            if можно_разместить_корабль(поле, x, y, размер_корабля, горизонтально):
                корабль = []
                for i in range(размер_корабля):
                    if горизонтально:
                        поле[x][y + i] = "🚢"
                        корабль.append((x, y + i))
                    else:
                        поле[x + i][y] = "🚢"
                        корабль.append((x + i, y))
                корабли.append(корабль)
                размещён = True
    
    # Скрываем корабли от игрока
    поле_игрока = [["🌊" for _ in range(10)] for _ in range(10)]
...
def можно_разместить_корабль(поле, x, y, размер, горизонтально):
    """Проверка возможности размещения корабля"""
    # Проверяем границы
    if горизонтально:
        if y + размер > 10: return False
    else:
        if x + размер > 10: return False
    
    # Проверяем занятость клеток и окружения
...
def создать_кнопки_морской_бой(поле):
    ...
    import telebot
    markup = telebot.types.InlineKeyboardMarkup(row_width=10)
    
    # Заголовок с буквами
    header = []
    for j in range(10):
        header.append(telebot.types.InlineKeyboardButton(chr(65 + j), callback_data="header"))
    markup.row(*header)
    
    # Поле с цифрами
    for i in range(10):
        row = [telebot.types.InlineKeyboardButton(str(i), callback_data="header")]
        for j in range(10):
            символ = поле[i][j]
            if символ == "🌊":
                text = "🌊"
                ...
                text = символ
            row.append(telebot.types.InlineKeyboardButton(text, callback_data=f"sea_{i}_{j}"))
        markup.row(*row)
    return markup

def обработать_ход_морской_бой(user_id, x, y):
    ...
    координата = chr(65 + y) + str(x)
    
    # Проверяем попадание
    if игра['корабли_поле'][x][y] == "🚢":
        игра['поле'][x][y] = "💥"
        результат = f"💥 Попадание! ({координата})"
        
        # Находим корабль и проверяем потопление
        for корабль in игра['корабли']:
            if (x, y) in корабль:
                ...
                if все_подбиты:
                    ...
                    for кx, кy in корабль:
                        игра['поле'][кx][кy] = "💀"
                    результат = f"💀 Потоплен! ({координата})"
                    игра['потоплено'] += 1
                break
    else:
        игра['поле'][x][y] = "❌"
        результат = f"💨 Мимо ({координата})"
    
    # Проверяем победу (10 кораблей потоплено)
    if игра['потоплено'] == 10:
        del игры_морской_бой[user_id]
        return None, f"{результат}\n🎉 Победа! Весь флот потоплен!\nДля новой игры напиши 'морской бой'"
    
    markup = создать_кнопки_морской_бой(игра['поле'])
    return markup, f"{результат}\nПотоплено: {игра['потоплено']}/10"
...
def запустить_бота():
    ...
    def echo_all(message):
        ...
        if результат == "hangman_start":
            ...
            игра = игры_морской_бой[user_id]
            markup = создать_кнопки_морской_бой(игра['поле'])
            bot.reply_to(message, "🚢 Классический морской бой!\nПоле 10x10, флот: 1×4, 2×3, 3×2, 4×1\nНажми на клетку для выстрела:", reply_markup=markup)
            return
    ...
    def handle_callback(call):
        ...
        if data.startswith("sea_"):
            if user_id in игры_морской_бой:
                _, x, y = data.split("_")
                markup, текст = обработать_ход_морской_бой(user_id, int(x), int(y))
                if markup:
